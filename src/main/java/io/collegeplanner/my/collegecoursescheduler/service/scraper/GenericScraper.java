...
public abstract class GenericScraper {
    ...
    public abstract void setTerm(final String season, String year);
    ...
    public final PermutationsJobResultsDto doPermutationsAndGetResults() {
        ...
        try {
            ...
            while(iterationVariables[0] < mostSignificantBit) {
                ...
                boolean schedulesCollide = false; // assume schedules don't collide (innocent until proven guilty)
...
//            printTables(validSchedules, false);
    ...
    public final void analyzePermutationsForTable() {

        final PermutationsJobResultsDto results;
        final long timeStartForPermutations = System.currentTimeMillis();
        final long MAX_TIMEOUT_IN_MS = TimeUnit.SECONDS.toMillis(8);

        try {
            createSizeSortedCourses();

            // Sort the course lists by size to allow for efficient dynamic programming techniques
            Collections.sort(sizeSortedCourses, (Comparator<List>) (a1, a2) -> {
                return a1.size() - a2.size();
            });

            final int numCourses = this.sizeSortedCourses.size();
            this.numChosenCourses = numCourses;

            // Better schedules "bubble up" with a limit of 25
            final PriorityQueue<ScheduleDto> validSchedules = new PriorityQueue<>();

            if(sizeSortedCourses.isEmpty()) {
                printTables(validSchedules, false);
                log.warn("sizeSortedCourses was empty");
            }
            ...
            while(iterationVariables[0] < (countIndexedCourses.get(0).size())) {
                if((System.currentTimeMillis() - timeStartForPermutations) > MAX_TIMEOUT_IN_MS) {
                    log.warn("Thread timed out during permutations");
                ...
                // TODO: too much nesting to read; needs cleaning up
                    ...
                    iterateBlocks:
                    // (numCourses - 2) because there's no point in storing the full permutation string
                    //     (e.g. if there's 5 classes then stored the iterations: 0000, 0001, 0002, 0010, etc.
                    //      DON'T store 00001 since it will only be used once and hence, doesn't save any time)
                    // (Math.max(..., 0) since if there's only one class, this will become an infinite loop
                    //     (plus it wouldn't help us with only one class anyway..)
                ...
                //     and class[(n-1)] timeblocks easily accessed from a map
                if(numCourses > 1) {
                    final int lastCourse = numCourses - 1;
                    // Checking against last course in permutation
                    for(int multipleDaySlots = 0; multipleDaySlots < countIndexedCourses.get(lastCourse).get(iterationVariables[lastCourse]).getDays().size(); multipleDaySlots++) {
                        final int[] daysArray = convDaysToArray(countIndexedCourses.get(lastCourse).get(iterationVariables[lastCourse]).getDays().get(multipleDaySlots));
                        for(int i = 0; i < daysArray.length; i++) {
            ...
            try {
                log.fatal("Error in analyzing the permutations" +
                                "Current Course ScheduleDto: {}", objectMapper.writeValueAsString(currentSchedule),
                        e);
            } catch (final JsonProcessingException err) {
                log.fatal("Error serializing course schedule", err);
            }
        }
        finally {
            log.info(String.format(
                    "[%s] Analyze job took %s seconds to finish %,d permutations.",
                    this.getUniversityName(), GenericUtils.getFormattedElapsedTime(timeStartForPermutations), getNumPermutations())
                    + "\t Course Selection: [" + String.join(", ", userOptions.getChosenCourseNames()) + "]"
            );
        }
    }
    */
    ...
    private final void printTables(final PriorityQueue<ScheduleDto> validSchedules, final boolean wasInterrupted) {
        final PrintWriter out = getOutWriter();
        ...
        out.println("<h2 class='numSched'>ScheduleDto <span id='numSched' class='numSched' data-count='1'>" + Math.min(1, size) + "</span> of " + Math.min(25, size) + "</h2>");
        if(!wasInterrupted) out.println("<i>There are " + NumberFormat.getNumberInstance(Locale.US).format(this.validSchedulesCount) + " valid permutations of your schedule - we filtered out " + NumberFormat.getNumberInstance(Locale.US).format((numPermutations - validSchedulesCount)) + " that didn't work</i>");
        else out.println("<i>There were too many permutations for your schedule so we optimized the first " + NumberFormat.getNumberInstance(Locale.US).format(numPermutations) + "." +  "<br>We found " + NumberFormat.getNumberInstance(Locale.US).format(this.validSchedulesCount) + " valid permutations of your schedule and filtered out " + NumberFormat.getNumberInstance(Locale.US).format((numPermutations - validSchedulesCount)) + " that didn't work</i>");
        out.println("<br>");
                        ...
                        for (int j = 0; j < sections; j++) {
                            ...
                            if((classDays[day] == 1) && (string_of_i.equals(course_time.substring(0, course_time.indexOf("-"))))) {
                                ...
                                // indented for ease of visualization
                                    ...
                                    out.println("<p id='schedNum'>ScheduleDto #: " + course.getScheduleNum() + "</p>");
                                out.println("</div>");
